...
MODEL_FILTER  = "claude-haiku-4-5-20251001"   # cheap pre-filter
MODEL_DETAIL  = "claude-sonnet-4-6"            # analysis + draft
...
def main():
    ...
    # 1. Broad keyword search across all inboxes
    all_threads = []
    seen = set()
    ...
    # 2. Claude filter — keeps only true events
    events = []
    for i, t in enumerate(all_threads):
        ...
        try:
            ...
            verdict = "EVENT"  # err on side of including
        if "EVENT" in verdict:
            events.append(t)
    ...
    # 3. Detailed analysis + draft per event
    results = []
